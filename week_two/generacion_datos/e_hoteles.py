...
from geopy.distance import geodesic  # Para calcular distancias reales en km 
import os 
...
ruta_salida = os.path.join(os.path.dirname(__file__), "hoteles_concentrados_centro.csv")
...
print(
    f"Archivo '{ruta_salida}' generado con {len(hotels)} hoteles concentrados cerca del centro de Madrid (media: {mean_dist_km} km, máx: {max_distance_km} km).")
...
plt.tight_layout()
# Guardar figura
ruta_salida_one = os.path.join(os.path.dirname(__file__), "distribucion_precios_hoteles.png")
plt.savefig(ruta_salida_one, dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
# Guardar figura
ruta_salida_two = os.path.join(os.path.dirname(__file__), "distribucion_distancias_centro.png")
plt.savefig(ruta_salida_two, dpi=300, bbox_inches='tight')
plt.show()
...
ax.set_axis_off()
# Guardar figura
ruta_salida_three = os.path.join(os.path.dirname(__file__), "hoteles_concentrados_sobre_mapa_centro_madrid.png")
plt.savefig(ruta_salida_three, dpi=300, bbox_inches='tight')
plt.show()
...
try:
    ruta_entrada = os.path.join(os.path.dirname(__file__), "estaciones_sistema.csv")
    df_metro = pd.read_csv(ruta_entrada)
    ...
    # Guardar figura
    ruta_salida_four = os.path.join(os.path.dirname(__file__), "hoteles_concentrados_centro_y_metro_madrid.png")
    plt.savefig(ruta_salida_four, dpi=300, bbox_inches='tight')
    plt.show()

...
- Cambio en la estructura JSON: En lugar de una lista para 'interactions', ahora es un objeto (dict) con claves numéricas:
  "interactions": {
    "interaccion_1": { "user_question": "...", "history": [...], "response": "..." },
    "interaccion_2": { ... },
    ...
  }
  Esto permite acceso directo por número de interacción (e.g., chat_data["interactions"]["interaccion_1"]).
- Se genera la clave automáticamente (e.g., "interaccion_1" para la primera, "interaccion_2" para la segunda, etc.).
- El guardado incremental lee el JSON, determina el siguiente número y agrega la nueva clave.
- Todas las demás funcionalidades permanecen iguales: archivos por chat, interfaz Gradio, generación, etc.
